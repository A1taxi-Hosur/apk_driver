/*
  # Fix get_driver_earnings_with_commission - Platform Fees Join

  ## Problem
  The function was trying to join on pf.vehicle_type which doesn't exist in platform_fees table.
  
  ## Solution
  Remove vehicle_type from the join condition, only join on booking_type.
*/
...
BEGIN
  ...
  SELECT 
    ...
    ROUND(COALESCE(pf.fee_amount, 0) * 0.18, 2) as gst_on_platform_fee,
    ROUND((r.fare_amount - COALESCE(pf.fee_amount, 0)) * 0.05, 2) as gst_on_charges,
    (r.fare_amount - COALESCE(pf.fee_amount, 0)) as ride_charges,
    ROUND((r.fare_amount - COALESCE(pf.fee_amount, 0)) * 0.11, 2) as commission_11_percent,
    ROUND(
      COALESCE(pf.fee_amount, 0) + 
      (COALESCE(pf.fee_amount, 0) * 0.18) + 
      ((r.fare_amount - COALESCE(pf.fee_amount, 0)) * 0.05) +
      ((r.fare_amount - COALESCE(pf.fee_amount, 0)) * 0.11),
      2
    ) as you_owe_total
  FROM rides r
  LEFT JOIN users u ON u.id = r.customer_id
  LEFT JOIN platform_fees pf ON pf.booking_type = r.booking_type AND pf.is_active = true
